...
const phase = computed(() => {
  ...
  if (state === 'CLAIM_CHALLENGE' || state === 'CLAIM_VERIFIED') return 'verify'
...
<template><ion-page><ion-header><ion-toolbar><ion-title>Add Device</ion-title></ion-toolbar></ion-header><ion-content>
  ...
  <div class="page">
    <ion-card v-if="!commissioning.mock"><ion-card-content><h2>Commissioning unavailable</h2><p class="muted">This build does not contain the native Matter commissioning plugin.</p></ion-card-content></ion-card>
    <template v-else>
      <ion-card><ion-card-content><p class="eyebrow">State {{ commissioning.state }}</p>
        ...
        <template v-else-if="phase === 'verify'"><h2>Verify ownership</h2><p>The development service will exercise claim and attestation UI states without handling real credentials.</p><ion-button expand="block" :disabled="commissioning.running" @click="commissioning.commission">Continue simulation</ion-button></template>
        ...
        <template v-else-if="phase === 'complete'"><h2>Complete</h2><p>The development wizard finished successfully. No physical device was changed.</p><ion-button expand="block" @click="commissioning.reset">Done</ion-button></template>
      ...
      <ion-card v-if="phase === 'prepare'"><ion-card-content><h3>Development failure injection</h3><ion-select v-model="commissioning.failureAt" label="Failure point" label-placement="stacked"><ion-select-option value="">None</ion-select-option><ion-select-option v-for="failure in failureOptions" :key="failure" :value="failure">{{ failure }}</ion-select-option></ion-select></ion-card-content></ion-card>
